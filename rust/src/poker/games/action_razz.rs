//! [wiki](https://github.com/lcrocker/ojpoker/wiki/Action_Razz) | Action razz low hands
...
pub fn ojp_ace_to_five_eval_full(h: &Hand) -> Result<HandDescription> {
    debug_assert!(ojp_ace_to_five_valid_hand(h));

    if h.len() > 5 {
        return ojp_best_of(h, Scale::AceToFive,
            ojp_ace_to_five_eval_5_full);
    } else if h.len() == 5 {
        return ojp_reference_evaluator_full(h, Scale::AceToFive);
    }
    ojp_ace_to_five_eval_5_full(h)
}
...
pub fn ojp_ace_to_five_eval_quick(h: &Hand) -> HandValue {
    ...
    if h.len() > 5 {
        return ojp_best_value_of(h, Scale::AceToFive,
            ojp_ace_to_five_eval_5_quick);
    } else if h.len() == 5 {
        return ojp_reference_evaluator_quick(h, Scale::AceToFive);
    }
    ojp_ace_to_five_eval_5_quick(h)
}
