...
fn curried_evaluator_high_full(h: &Hand) -> Result<HandValue, OjError> {
    ojp_default_eval_full(h, Scale::HighHand)
}

fn curried_evaluator_high_quick(h: &Hand) -> u32 {
    ojp_default_eval_quick(h, Scale::HighHand)
}
...
pub fn ojp_high_eval_full(h: &Hand) -> Result<HandValue, OjError> {
    if h.len() < 5 {
        return curried_evaluator_high_full(h);
    }
    let ec = if 5 == h.len() {
        lookup_high(h)
    } else {
...
pub fn ojp_high_eval_quick(h: &Hand) -> u32 {
    ...
    if h.len() < 5 {
        return curried_evaluator_high_quick(h);
    }
    ojp_best_value_of(h, Scale::HighHand, lookup_high)
}
...
fn curried_evaluator_stripped_full(h: &Hand) -> Result<HandValue, OjError> {
    ojp_default_eval_full(h, Scale::Stripped)
}

fn curried_evaluator_stripped_quick(h: &Hand) -> u32 {
    ojp_default_eval_quick(h, Scale::Stripped)
}
...
pub fn ojp_stripped_eval_full(h: &Hand) -> Result<HandValue, OjError> {
    ...
    let ec = if 5 == h.len() {
        ...
        ojp_best_value_of(h, Scale::Stripped, lookup_stripped)
    };
    let vv = STRIPPED_DECK_TABLE_2[ec as usize];
    let mut v = HandValue::new_with_value(*h, Scale::Stripped,
        vv.0, ec as u32);
    v.order_for_display(&vv.1);
    Ok(v)
}
...
/// use onejoker::*;
///
/// let hand = Hand::new(DeckType::Stripped).init(hand!("9s","As","9d","Ks","Ah"));
...
pub fn ojp_stripped_eval_full(h: &Hand) -> Result<HandValue, OjError> {
    if h.len() > 5 {
        return ojp_best_of(h, Scale::Stripped,
...
/// use onejoker::*;
///
/// let h1 = Hand::new(DeckType::Stripped).init(hand!("9s","As","9d","Ks","Ah"));
/// let h2 = Hand::new(DeckType::Stripped).init(hand!("9c","Ac","9h","Td","Ad"));
...
pub fn ojp_stripped_eval_quick(h: &Hand) -> u32 {
    if h.len() > 5 {
        return ojp_best_value_of(h, Scale::Stripped,
            curried_evaluator_stripped_quick);
    }
    curried_evaluator_stripped_quick(h)
}

fn curried_evaluator_high_bug_full(h: &Hand) -> Result<HandValue, OjError> {
