...
void main(List<String> arguments) {
  List<HandValueHigh> allHands = [];
  Map<int, int> hashMap = {};
  Map<int, HandValueHigh> valueMap = {};

  var heap = MinMaxHeap<HandValueHigh>(criteria: (HandValueHigh v) => v.value);
  var deck = Deck("poker");
  ...
  for (Hand hand in deck.combinations(5)) {
    ...
    HandValueHigh v = HandValueHigh.referenceEvaluator(hand);
    heap.add(v);
  }
  if (2598960 != total) {
  ...
  while (heap.isNotEmpty) {
    HandValueHigh v = heap.removeMin();
    allHands.add(v);
  }
  int equiv = 0;
  int pValue = 0;
  ...
  for (HandValueHigh v in allHands) {
    if (v.value != pValue) {
      equiv += 1;
    }
  ...
  var file = File('../data/json/high_hand_prime_hash.json5');
